...
//		})	
	...
	$("#writeAddress").on("blur",function(e){
		...
		if(writeAddress.length>0 && writeAddress.length<=30){			
	...
	$("#writeProduct").on("blur",function(e){
		...
		if(writeProduct.length>0 && writeProduct.length<=50){
	...
	$("#writeNum").on("blur",function(e){
		...
		if(writeNum.length>0 && writeNum.length<=4){
	...
	$("#writeMore").on("blur",function(e){
		...
		if(writeMore.length>0 && writeMore.length<=200){
	...
	$("#agencyConsumer").on("blur",function(e){
		...
		if(agencyConsumer.length>0 && agencyConsumer.length<=5){
	...
	$("#agencyAddress").on("blur",function(e){
		...
		if(agencyAddress.length>0 && agencyAddress.length<=30){
	...
	$("#agencyName").on("blur",function(e){
		...
		if(agencyName.length>0 && agencyName.length<=5){
			...
			cont("含有非法字符且您的姓名至多5个字")
			showAddhide()
			/*WPBridge.callMethod("JsInvokeNative", "wpShowToast", {
                    message: "含有非法字符且您的姓名至多5个字"
                },
                function() {})*/
			wrongColor("#writeName",".name")
		}else{
	...
	$("#agencyModel").on("blur",function(e){
		...
		if(agencyModel.length>0 && agencyModel.length<=50){
	...
	$("#agencyNumber").on("blur",function(e){
		...
		if(agencyNumber.length>0 && agencyNumber.length<=4){
	...
	$("#agencyRemark").on("blur",function(e){
		...
		if(agencyRemark.length>0 && agencyRemark.length<=200){
